...
public:
    ...
    cuda_video(bool gpu, unsigned int deviceIndex = 0);
    ~cuda_video();

    // Equivalent to 'init' in OpenCL code; in CUDA, we typically don't build kernels at runtime,
    // but we keep a method here in case you want to load constants or do any setup.
    void init(const std::string& /*unused*/);
