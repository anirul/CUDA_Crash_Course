...
// --------------------------------------------------------
//   cuda_video class Implementation
// --------------------------------------------------------
cuda_video::cuda_video(bool gpu, unsigned int deviceIndex)
    : gpu_(gpu)
...
void cuda_video::selectDevice(unsigned int deviceIndex)
{
    int deviceCount = 0;
...
void cuda_video::init(const std::string& /*unused*/)
{
    // In OpenCL you compiled kernels from a .cl file at runtime.
    // In CUDA, we typically compile .cu ahead-of-time with NVCC.
    // So this method may be no-op or used for something else if needed.
}

void cuda_video::setup(const std::pair<unsigned int, unsigned int>& size,
                       unsigned int nb_col)
{
    width_  = size.first;
    height_ = size.second;
...
void cuda_video::prepare(const std::vector<char>& input)
{
...
std::chrono::nanoseconds cuda_video::run(std::vector<char>& output)
{
